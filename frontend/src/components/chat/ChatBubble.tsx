...
const ChatBubble = ({ message, image, isSelf, timestamp }: ChatBubbleProps) => {
        ...
        <div
          ...
          {image && (
            <img
              src={image}
              alt="Shared image"
              className="w-full max-w-xs object-cover rounded-sm"
            />
          )}
          {message && (
            <p className={`py-2 px-4 font-light ${image ? "border-t border-white/10" : ""}`}>
              {message}
            </p>
          )}
        </div>

        <span className={`block text-[10px] text-foreground/80 mt-1 ${isSelf ? "text-right mr-1" : "text-left ml-1"}`}>
          {timestamp}
        </span>
      </div>
    </div>
